...
from schedule_io import load_schedule, save_schedule
...
# JRAの重賞は年をまたいで同じレース名が繰り返されるため、
# schedule.json に既にある実績(work_slug)を「レース名の核 -> slug」の
# 辞書として使い、新規検出時もここから自動解決する。
# ここに無い(=本当に初出の)レースだけ work_slug が空欄になる。
KNOWN_SLUGS = {
    # 既存
    "フェブラリー": "february-stakes", "高松宮記念": "takamatsu-no-miya-kinen",
    "大阪杯": "osaka-hai", "桜花賞": "oka-sho", "皐月賞": "satsuki-sho",
    "天皇賞春": "tenno-sho-spring", "NHKマイルC": "nhk-mile-cup",
    "ヴィクトリアM": "victoria-mile", "優駿牝馬": "oaks", "ダービー": "derby",
    "目黒記念": "meguro-kinen", "安田記念": "yasuda-kinen", "宝塚記念": "takarazuka-kinen",
    "府中牝馬": "fuchu-himba-stakes", "しらさぎ": "shirasagi-stakes",
    "函館記念": "hakodate-kinen", "北九州記念": "kitakyushu-kinen", "七夕賞": "tanabata-sho",
    "小倉記念": "kokura-kinen", "関屋記念": "sekiya-kinen", "クイーン": "queen-stakes",
    "CBC賞": "cbc-sho", "中京記念": "chukyo-kinen", "札幌記念": "sapporo-kinen",
    "キーンランドC": "keeneland-cup", "新潟記念": "niigata-kinen",
    "セントウル": "centaur-stakes", "セントライト記念": "kobe-shimbun-hai",
    "オールカマー": "all-comers", "スプリンターズ": "sprinters-stakes",
    "毎日王冠": "mainichi-osho", "秋華賞": "shuka-sho", "菊花賞": "kikka-sho",
    "天皇賞秋": "tenno-sho-autumn", "アルゼンチン共和国杯": "argentina-kyowakoku-hai",
    "エリザベス女王杯": "queen-elizabeth-2-cup", "マイルCS": "mile-championship",
    "ジャパンC": "japan-cup", "チャンピオンズC": "champions-cup",
    "阪神JF": "hanshin-juvenile-fillies", "朝日杯FS": "asahi-hai-futurity-stakes",
    "有馬記念": "arima-kinen",

    # 追加（2026年重賞・OP・ジャンプ）
    "中山金杯": "nakayama-kinpai",
    "京都金杯": "kyoto-kinpai",
...
KNOWN_SLUGS_NORM = {_race_core(k): v for k, v in KNOWN_SLUGS.items()}


def resolve_work_slug(race_name):
    """レース名から work_slug を解決する。分からなければ None を返す。"""
    core = _race_core(race_name)
    if core in KNOWN_SLUGS_NORM:
        return KNOWN_SLUGS_NORM[core]
    # 前方一致・部分一致でも軽く探す(表記ゆれ対策)
    for k, v in KNOWN_SLUGS_NORM.items():
...
def main():
    ...
    for r in scraped:
        ...
        slug = resolve_work_slug(r["race_name"])
        schedule.setdefault("races", []).append({
            ...
            "work_slug": slug or "",
            "active": False,
            "memo": "" if slug else "要work_slug設定",
        })
        existing_keys.add(key)
        added.append((r["race_name"], bool(slug)))

    if added:
        save_schedule(schedule, SCHEDULE_PATH)
        resolved = [n for n, ok in added if ok]
        unresolved = [n for n, ok in added if not ok]
        if resolved:
            print("新規追加(work_slug自動解決): " + ", ".join(resolved))
        if unresolved:
            print("新規追加(work_slug要手動設定): " + ", ".join(unresolved))
    else:
